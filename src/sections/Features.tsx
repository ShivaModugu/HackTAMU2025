  ...
  return (
    ...
    >
      {props.selected && (
        <motion.div style={{
          maskImage,
        }}className="absolute inset-0 -m-px border border-[#FF6B6B] rounded-xl"></motion.div>
      )}
      ...
      {props.isNew && <div className="text-xs rounded-full px-2 py-0. bg-[#FF4C4C] text-black font-semibold">new</div>}
    </div>
  )
}
...
export const Features = () => {
  ...
  const handleSelectTab = (index:number) => {
    ...
    animate(backgroundSizeX, [backgroundSizeX.get(), 100, tabs[index].backgroundSizeX], animatedOptions);
    animate(backgroundPositionX, [backgroundPositionX.get(), tabs[index].backgroundPositionX],animatedOptions);
    animate(backgroundPositionY, [backgroundPositionY.get(), tabs[index].backgroundPositionY],animatedOptions);
  };

  return (
  <section className="py-20 md:py-24">
    <div className="container">
      <h2 className="text-5xl md:text-6xl font-medium text-center tracking-tighter">Level up your ride</h2>
